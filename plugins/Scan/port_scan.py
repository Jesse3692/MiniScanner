# coding:utf-8
# 参考 https://raw.githubusercontent.com/AnthraX1/InsightScan/master/scanner.py
# 端口扫描
...
def get_server(port):
    SERVER = {
        ...
        '68': 'DHCP',
        '69': 'TFTP',  # Trival File Transfer
        '79': 'Finger Server',
        '80': 'HTTP',
        '99': 'Gram Relay',
        '102': 'MTA',  # 消息传输代理(MTA)-X.400 over TCP/IP
        '109': 'Post Office Protocol -Version3',  # POP3服务器开放此端口，用于接收邮件，客户端访问服务器端的邮件服务 # noqa:E501
        ...
        '389': 'LDAP',
        '443': 'HTTPS',
        '456': 'WebDAV',  # 木马HACKERS PARADISE开放此端口
        '489': 'LDAP',
        '445': 'SMB',
        '465': 'SMTPS',
        '512': 'Linux R RPE',
        ...
        '993': 'IMAPS',
        '995': 'POP3',
        '1001': 'Trojan',  # 木马Silencer、WebEx开放1001端口
        '1011': 'Trojan',  # 木马Doly Trojan开放1011端口
        '1080': 'Proxy',
        '1099': 'JavaRMI',
        '1352': 'Lotus',
        '1433': 'MSSQL',
...
PORTS = [
    21, 22, 23, 25, 26, 31, 37, 42, 47, 49, 53, 67, 68, 69, 70, 79, 80, 81,
    82, 83, 84, 88, 89, 99, 102, 109, 110, 111, 119, 123, 129, 135, 137,
    138, 139, 143, 146, 161, 163, 175, 177, 179, 195, 199, 222, 258, 259,
    264, 280, 301, 306, 311, 340, 366, 389, 425, 427, 443, 444, 445, 456,
    458, 465, 481, 489, 497, 500, 502, 503, 512, 513, 514, 515, 520, 523,
    524, 530, 541, 544, 548, 553, 554, 555, 568, 569, 635, 666, 726, 749,
    751, 765, 771, 777, 783, 787, 789, 808, 843, 873, 880, 888, 898, 901,
    902, 981, 987, 990, 992, 993, 995, 996, 999, 1000, 1001, 1007, 1010,
    1011, 1021, 1023, 1024, 1025, 1080, 1088, 1099, 1102, 1111, 1117, 1119,
    1126, 1141, 1325, 1328, 1334, 1352, 1400, 1417, 1433, 1434, 1443, 1455,
    1461, 1471, 1494, 1503, 1515, 1521, 1524, 1533, 1723, 2082, 2083, 2179,
    2181, 2196, 2200, 2222, 2251, 2260, 2288, 2301, 2323, 2332, 2333, 2366,
    2375, 2376, 2379, 2399, 2401, 2404, 2433, 2455, 2480, 2492, 2500, 2522,
    2525, 2557, 2601, 2604, 2628, 2638, 2710, 2725, 2800, 2809, 2811, 2869,
    2875, 2920, 2998, 3000, 3001, 3003, 3011, 3013, 3017, 3052, 3071, 3077,
    3128, 3168, 3211, 3221, 3260, 3269, 3283, 3299, 3306, 3307, 3310, 3311,
    3312, 3333, 3351, 3367, 3386, 3388, 3389, 3404, 3460, 3476, 3478, 3493,
    3517, 3527, 3541, 3542, 3546, 3551, 3560, 3580, 3659, 3661, 3689, 3690,
    3702, 3703, 3737, 3749, 3766, 3780, 3784, 3790, 3794, 3809, 3814, 3851,
    3869, 3871, 3878, 3880, 3889, 3905, 3914, 3918, 3920, 3945, 3971, 3986,
    3995, 3998, 4000, 4022, 4040, 4045, 4063, 4064, 4070, 4111, 4129, 4200,
    4224, 4242, 4279, 4321, 4343, 4440, 4848, 5432, 5550, 5555, 5560, 5566,
    5577, 5601, 5631, 5632, 5633, 5666, 5672, 5683, 5718, 5730, 5800, 5801,
    5815, 5822, 5825, 5850, 5858, 5859, 5862, 5877, 5900, 5901, 5915, 5922,
    5925, 5938, 5950, 5952, 5984, 5985, 5986, 6000, 6082, 6379, 7001, 7002,
    7004, 7007, 7019, 7025, 7070, 7071, 7080, 7100, 7103, 7106, 7218, 7402,
    7435, 7443, 7474, 7496, 7512, 7547, 7548, 7549, 7625, 7627, 7657, 7676,
    7741, 7777, 7778, 7779, 7800, 7903, 7905, 7911, 8000, 8001, 8008, 8009,
    8010, 8031, 8042, 8045, 8060, 8069, 8080, 8081, 8082, 8083, 8086, 8087,
    8088, 8089, 8090, 8091, 8093, 8098, 8099, 8112, 8126, 8139, 8140, 8161,
    8181, 8191, 8200, 8222, 8254, 8291, 8300, 8307, 8333, 8334, 8383, 8390,
    8400, 8402, 8433, 8443, 8500, 8554, 8600, 8649, 8654, 8688, 8701, 8800,
    8834, 8873, 8880, 8883, 8888, 8889, 8899, 9001, 9151, 9160, 9191, 9200,
    9207, 9220, 9290, 9300, 9306, 9415, 9418, 9443, 9485, 9500, 9535, 9575,
    9595, 9600, 9618, 9666, 9869, 9898, 9900, 9903, 9917, 9929, 9943, 9944,
    9968, 9981, 9990, 9998, 9999, 10000, 10001, 10012, 10050, 10051, 10082,
    10180, 10215, 10243, 10554, 10566, 10621, 10626, 10778, 11211, 11300,
    11967, 12000, 12124, 12174, 12265, 12345, 12888, 13456, 13579, 13722,
    14000, 14003, 14147, 14238, 15000, 15660, 15742, 16010, 16012, 21571,
    22022, 22222, 22939, 23023, 23307, 23389, 23424, 23502, 24212, 24444,
    24800, 25105, 25565, 26214, 27000, 27015, 27016, 27017, 27018, 27019,
    27545, 27715, 28015, 28017, 28201, 28561, 30000, 30718, 30951, 31038,
    31337, 50070
]
...
class ScanPort:
    def __init__(self, ipaddr, dbname):
        self.ipaddr = ipaddr    # 扫描地址
        self.port = []  # 开放端口
        self.dbname = dbname    # 存入数据库名称
        self.out = []   # 记录端口,对应服务与banner信息
        self.num = 0    # 判断port spoof的标记值

    # 数据存储
    def save(self, ipaddr, result):
        Sqldb(self.dbname).set_ports(ipaddr, result)
    ...
    def regex(self, response, port):
        ...
        for pattern in SIGNS:
            ...
            if re.search(pattern[-1], response, re.IGNORECASE):
                ...
                match = True
                proto = {"server": pattern[1].decode(
                ), "port": port, "banner": text}
                self.out.append(proto)
                break
        if not match:   # 响应特征无法判断时根据常用端口服务判断
            proto = {"server": get_server(
                port), "port": port, "banner": response.decode('utf-8', 'ignore')}
            self.out.append(proto)
    ...
    def socket_scan(self, hosts):
        ...
        try:
            ...
            # 开放端口数目大于30时，很可能是port spoof,改变标记值
            else:
                self.num = 1
    ...
    def run(self, ip):
        hosts = []
        global PORTS
        random.shuffle(PORTS)   # 将端口随机打乱后与ip组合
        for i in PORTS:
            hosts.append('{}:{}'.format(ip, i))
        try:
            # 使用线程池，线程数最大为100
            # future_row = {} for v in range(n_data): future_row[executor.submit(self.myjob, v)] = v
            # An iterator over the given futures that yields each as it completes.
            with concurrent.futures.ThreadPoolExecutor(max_workers=100) as executor:
                result = {executor.submit(
                    self.socket_scan, i): i for i in hosts}
                for future in concurrent.futures.as_completed(result, timeout=3):
                    future.result()
                    # 判断为port spoof后停止并退出线程
                    if self.num == 1:
                        break
    ...
    def pool(self):
        ...
        try:
            ...
            if re.search(r'\d+\.\d+\.\d+\.\d+', self.ipaddr):
                ...
                ipaddr = socket.gethostbyname(self.ipaddr)  # 根据域名获取ip地址
            if ':' in ipaddr:
        ...
        if self.num == 0:
            ...
            for _ in self.out:
                ...
                console('ScanPort', self.ipaddr, '{}:{}\n'.format(
                    _.get('server'), _.get('port')))
            return out
        else:
            # 服务记作port spoof存入数据库
            self.save(self.ipaddr, [
                      {"server": 'port spoof', "port": '0', "banner": ''}])
            console('ScanPort', self.ipaddr, 'port spoof:0\n')
            return ['port spoof:0']


if __name__ == "__main__":
    start_time = time.time()
    ScanPort('www.baidu.com', 'port_result').pool()
    end_time = time.time()
    print('\nrunning {0:.3f} seconds...'.format(end_time - start_time))
    print(Sqldb('port_result').query('select * from ports'))
